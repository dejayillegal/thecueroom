/**
 * Insert comprehensive test posts into TheCueRoom database
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'Cookie': 'connect.sid=s%3AaLx0E8_HZR2u3gKbqp7XDqM5F9KvIcEr.YourSessionIdHere'
      }
    };
...
async function insertAllPosts() {
  console.log(`Inserting ${testPosts.length} test posts...`);
  
  for (let i = 0; i < testPosts.length; i++) {
    try {
      const result = await makeRequest(testPosts[i]);
      console.log(`✓ Post ${i + 1}/${testPosts.length}: "${testPosts[i].title}"`);
    } catch (error) {
      console.log(`✗ Post ${i + 1}/${testPosts.length}: ${error.message}`);
    }
    // Small delay to avoid overwhelming server
    await new Promise(resolve => setTimeout(resolve, 100));
  }
  
  console.log("Finished inserting test posts!");
}

insertAllPosts().catch(console.error);